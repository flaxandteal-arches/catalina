    ...
    */
    ...
    getDisplayName: function(node) {
        ...
        return ko.computed(function(){
            var name = node.name();
            // Prefer the node's widget label (translatable) over the node name (internal identifier).
            var widget = self.graphModel.get('cardwidgets')().find(function(w){
                return w.node_id === node.nodeid;
            });
            if (widget && widget.label) {
                var label = widget.label;
                if (typeof label === 'object') {
                    // raw i18n dict {lang: value}; resolve to active language with fallback
                    label = label[arches.activeLanguage] || Object.values(label)[0];
                }
                if (label) {
                    name = label;
                }
